...
} cache_line_t; // renames the struct

typedef struct fifo_cache_set {
    cache_line_t* front; // front (head) of the queue
    cache_line_t* back; // back (tail) of the queue
    unsigned char occupancy;
} fully_associative_fifo_cache_t; //renames the struct

void enqueue ( fully_associative_fifo_cache_t* fully_associative_fifo_cache_t, mem_addr_t tag ) {
    ...
    if (fully_associative_fifo_cache_t->back==NULL) { // If the Queue is currently empty
        fully_associative_fifo_cache_t->front = queueNode;
        fully_associative_fifo_cache_t->back = queueNode;
    } else {
        fully_associative_fifo_cache_t->back->next_cache_line = queueNode;
        fully_associative_fifo_cache_t->back = queueNode;
    }
...
mem_addr_t dequeue ( fully_associative_fifo_cache_t* fully_associative_fifo_cache_t) {

    if (fully_associative_fifo_cache_t->front==NULL) { // If the Queue is currently empty
        return '\0';
    } else {

        // The QueueNode at front of the queue to be removed
        cache_line_t* temp = fully_associative_fifo_cache_t->front;
        mem_addr_t data = temp-> tag;

        fully_associative_fifo_cache_t->front = temp->next_cache_line;
        if (fully_associative_fifo_cache_t->back==temp) { // If the Queue will become empty
            fully_associative_fifo_cache_t->back = NULL;
        }
