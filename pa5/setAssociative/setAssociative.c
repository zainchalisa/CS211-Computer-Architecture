...
void accessData (
    ...
    cache_line_t* lru = &cache[index][0];

    // Find the correct cache set
    for( int i = 0; i < 4; i++){
        if(tag == cache[index][i].tag && cache[index][i].valid == 1){
        // Cache Hit
            (*hit_count)++;
    ...
    for( int i = 0; i < 4; i++){
        if(lru->recent_use > cache[index][i].recent_use){ // finding the minimum amount you need to realize that what your comparing to is the value make sure you check the conditional is doing so
            lru = &cache[index][i];
        }
    }
        ...
        // Otherwise Cache Miss
            ...
            lru->valid = 1;
            lru->tag = tag;
            lru->recent_use = program_counter;
            //printf("miss");
        } else{
            (*eviction_count)++;
            lru->tag = tag;
